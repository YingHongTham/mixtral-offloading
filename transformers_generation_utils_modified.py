## modified version of ~/myenv01/lib/python3.10/site-packages/transformers/generation/utils.py
## so can add nvtx range around each token generation
...
class GenerationMixin:
    ...
    def sample(
        ...
        **model_kwargs,
        ...
        this_peer_finished = False  # used by synced_gpus only
        token_count = 0 ## YH added
        # auto-regressive generation
        while True:
            token_count += 1 ## YH added
            nvtx.range_push(f"token_{token_count:03}") ## YH added
            if synced_gpus:
                # Under synced_gpus the `forward` call must continue until all gpus complete their sequence.
                # The following logic allows an early break if all peers finished generating their sequence
                this_peer_finished_flag = torch.tensor(0.0 if this_peer_finished else 1.0).to(input_ids.device)
            ...
            # forward pass to get next token
            nvtx.range_push(f"forward_{token_count:03}") ## YH added
            outputs = self(
                **model_inputs,
                return_dict=True,
                output_attentions=output_attentions,
